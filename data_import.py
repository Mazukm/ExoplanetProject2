...
class DataImport():
    ...
    def avrg_num_metalicity(df):
        """
        calculates the avrage number of planets according to each metalicity column
        """
        return df[df["st_metratio"] != "[Fe/H["].groupby("st_metratio", as_index=False)["sy_pnum"].mean()
    ...
    def read_in_data():
        ...
        data_frame1 = pd.read_csv(current_dir + "/" + "PS_2022.05.23_16.34.26.csv", skiprows=99)
        data_frame2 = pd.read_csv(current_dir + "/" + "PS_2022.05.23_16.36.33.csv", skiprows=96)
