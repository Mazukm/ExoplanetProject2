"""
CSE 163 SCHAFER
proceeses creates saves and verifies 
a random tree regressor model
"""
import pandas as pd
import numpy as np
...
class MachineLearning():
    ...
    def prep_data(df):
        ...
        features = pd.get_dummies(
            features, columns=["st_metratio", "st_spectype"])
    ...
    def train_model(labels_train, features_train):
        ...
        """
        model = RandomForestRegressor().fit(features_train,
                                            np.array(labels_train))
        return model

    def test_model(labels_test, features_test, model):
        """
        runs the given model, and calculates the accuracy from taking the
        correct labels and comparing to the output
        """
        test_predictions = model.predict(features_test)
        print('Test  Deviation:', mean_squared_error(
            np.array(labels_test).reshape(-1, 1), test_predictions))
